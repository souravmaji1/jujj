    ...
    <AbsoluteFill style={{ backgroundColor: '#111827' }}>
      {clips && clips.length > 0 ? (
        clips.reduce((acc, clip, index) => {
          // Calculate duration of the current clip
          const durationInSeconds = Math.max(clip.end - clip.start, 1 / FPS);
          const durationInFrames = Math.round(durationInSeconds * FPS);

          // Calculate startFrame as the sum of all previous clips' durations
          const startFrame = acc.previousDuration;

          // Add to the cumulative duration for the next clip
          acc.sequences.push(
            <Sequence key={index} from={startFrame} durationInFrames={durationInFrames}>
              <RemotionVideo
                src={clip.src}
                startFrom={0} // Play from the beginning of the clip
          ...
          acc.previousDuration += durationInFrames;

          return acc;
        }, { sequences: [], previousDuration: 0 }).sequences
      ) : (
        <div
          style={{
            width: '100%',
...
export default function VideoCreatorPage() {
  ...
  // Function to handle video rendering and YouTube upload
  const handleRenderVideo = async () => {
    ...
    try {
      ...
      // Step 2: Fetch Google access token from Supabase
      ...
      if (googleAccountError || !googleAccount?.access_token) {
        throw new Error('No Google account connected or access token unavailable. Please connect your Google account in settings.');
      }

      // Step 3: Prepare FormData for YouTube upload
      const formData = new FormData();
      formData.append('video', blob, fileName);
      formData.append('title', selectedCampaign?.name || 'My Video Reel');
      formData.append('description', selectedCampaign?.description || 'Created with VideoSync AI Video Platform');
      ...
      console.log('Video uploaded to YouTube:', uploadData);

      // Step 5: Update campaign with YouTube video ID and influencer ID
      if (selectedCampaign?.id && uploadData.youtubeVideoId && userChannelId) {
        // Fetch existing youtube_videos or initialize as empty array
        ...
        if (campaignFetchError) {
          throw new Error(`Failed to fetch campaign data: ${campaignFetchError.message}`);
        }

        const currentVideos = Array.isArray(campaignData.youtube_videos) 
          ? campaignData.youtube_videos 
          : [];

        // Check if the video ID already exists to avoid duplicates
        const videoExists = currentVideos.some(video => video.video_id === uploadData.youtubeVideoId);
        if (!videoExists) {
          const newVideoEntry = {
            video_id: uploadData.youtubeVideoId,
            influencer_id: userChannelId
          };
          const updatedVideos = [...currentVideos, newVideoEntry];

          const { error: updateError } = await supabase
            .from('campaigns')
            .update({ youtube_videos: updatedVideos })
            .eq('id', selectedCampaign.id);

          if (updateError) {
            throw new Error(`Failed to update campaign with video and influencer data: ${updateError.message}`);
          }
          ...
          setCampaigns(prevCampaigns =>
            prevCampaigns.map(campaign =>
              campaign.id === selectedCampaign.id
                ? { ...campaign, youtube_videos: updatedVideos }
                : campaign
            )
          );
      ...
      // Step 6: Optionally, allow downloading the video
      ...
      setRenderError(null);
      alert(`Video successfully uploaded to YouTube! Video ID: ${uploadData.youtubeVideoId}`);
    } catch (error) {
      console.error('Error rendering, uploading, or updating campaign:', error);
      setRenderError(error.message || 'Failed to render, upload video, or update campaign');
    } finally {
      setRendering(false);
    }
  };
                       ...
                       <div style={{display: 'flex',justifyContent:'center',margin:'54px 0 0',position:'relative'}}>
                        <Button
                            ...
                            >
                              ...
                              {rendering ? 'Rendering...' : 'Render & Upload to YouTube'}
                            </Button>
                            </div>
                      </motion.div>
