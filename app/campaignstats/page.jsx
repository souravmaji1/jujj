...
import {
  Home, Settings, HelpCircle, ChevronRight, ChevronDown, Video, BookOpen, Music, Users,SquarePlus,
  Sparkles, Zap, Plus, X, User, Briefcase, Star, BarChart, Eye, Heart, MessageCircle, ChartColumnDecreasing,
  Calendar, Link as LinkIcon, ChevronUp, ChevronDown as ChevronDownIcon
} from 'lucide-react';
...
// Mock data for view trends
const generateMockTrendData = () => [
  { day: 'Day 1', views: 100 }, { day: 'Day 2', views: 150 }, { day: 'Day 3', views: 200 },
  { day: 'Day 4', views: 300 }, { day: 'Day 5', views: 250 }, { day: 'Day 6', views: 400 },
  { day: 'Day 7', views: 350 }
];
...
export default function YouTubeAnalyticsPage() {
  ...
  const [userRole, setUserRole] = useState('buyer'); // Assume creator is a 'buyer'
  const [isAnimating, setIsAnimating] = useState(false);
  const [influencers, setInfluencers] = useState({});
  const [expandedCampaigns, setExpandedCampaigns] = useState({});

  // Fetch campaigns created by the user and their video analytics
  useEffect(() => {
    if (!user) return;
      ...
      try {
        ...
        const { data: campaignData, error: campaignError } = await supabase
          ...
          .select(`
            ...
            user_id
          `)
          .eq('user_id', user.id); // Filter by creator's user_id

        if (campaignError) throw new Error(`Failed to fetch campaigns: ${campaignError.message}`);

        if (!campaignData || campaignData.length === 0) {
          setError('No campaigns found created by you');
          setCampaigns([]);
          setLoading(false);
          return;
        }
        ...
        // Fetch influencer details for all selected influencers
        ...
        // Collect all video IDs from campaigns
        const videoIds = campaignData
          .flatMap(campaign => (campaign.youtube_videos || []).map(video => video.video_id))
          .join(',');
  ...
  // Calculate total stats across all videos
        ...
        <ScrollArea className="flex-1 py-6 px-4">
                ...
                <div className="flex space-x-2 mb-4">
                  <Button
                    ...
                    className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all
                      ${userRole === 'buyer' ? 'bg-gradient-to-r from-purple-700 to-blue-700 text-white shadow-md' : 'bg-gray-800/70 text-gray-300 hover:bg-gray-700'}`}
                  >
                  ...
                  <Button
                    ...
                    className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all
                      ${userRole === 'seller' ? 'bg-gradient-to-r from-purple-700 to-blue-700 text-white shadow-md' : 'bg-gray-800/70 text-gray-300 hover:bg-gray-700'}`}
                  >
          ...
          {userRole === 'buyer' && (
            <div className={`mb-8 transition-all duration-500 delay-100 ${isAnimating ? 'opacity-0 transform translate-y-4' : 'opacity-100 transform translate-y-0'}`}>
              {sidebarOpen && (
                <div className="flex items-center justify-between px-4 mb-4">
                  <h3 className="text-xs uppercase text-gray-400 font-semibold tracking-wider">Marketplace</h3>
                  <div className="w-8 h-0.5 bg-gray-800/70 rounded-full"></div>
                </div>
              )}
              <ul className="space-y-2">
          ...
          <div className="max-w-7xl mx-auto p-6 md:p-8">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div className="mb-6 md:mb-0">
                      ...
                      <p className="text-lg text-gray-200 max-w-xl">
                        Monitor the performance of all videos in your campaigns with detailed analytics.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {!loading && campaigns.length === 0 && (
              <div className="text-center text-gray-300 py-12">
                ...
                <p>No campaigns found created by you.</p>
                <Link href="/create" passHref>
            ...
            {!loading && campaigns.length > 0 && (
              <div className="space-y-6">
                {campaigns.map(campaign => {
                  const videos = campaign.youtube_videos || [];
                          ...
                          >
                            <div className="p-6 pt-0 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                              {videos.map(video => {
                                ...
                                const influencer = influencers[video.influencer_id] || {};
                                return (
                                  <motion.div
                                    key={video.video_id}
                                  ...
                                  >
                                    ...
                                    <div className="relative z-10">
                                      ...
                                      <p className="text-sm text-gray-300 mb-3">
                                        By: {influencer.channel_title || 'Unknown Influencer'}
                                      </p>
