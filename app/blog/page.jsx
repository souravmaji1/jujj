...
export default function SettingsPage() {
  ...
  const [campaignData, setCampaignData] = useState({
    ...
    artistName: '',
    artistBio: '',
    content: [], // Changed to array for multiple files
    selectedAccounts: []
  });
  ...
  const toggleAccountSelection = (channelId) => {
    setCampaignData(prev => {
      const selectedAccounts = prev.selectedAccounts.includes(channelId)
        ? prev.selectedAccounts.filter(id => id !== channelId)
        : [...prev.selectedAccounts, channelId];
        ...
        .insert([
          {
            ...
            selected_influencers: campaignData.selectedAccounts
          }
        ])
        .select()
        .single();
          ...
          <div className="max-w-6xl mx-auto p-6">
            ...
            {step === 4 && (
              <div className="bg-gray-900/60 backdrop-blur-md border border-gray-800/50 rounded-2xl p-8 shadow-xl shadow-purple-900/5 animate-fadeIn">
                ...
                <p className="text-gray-300 mb-6">Choose the YouTube channels that will promote your music.</p>
                  ...
                  {youtubeInfluencers.map(influencer => (
                    <div 
                      ...
                      className={`relative p-4 rounded-xl border transition-all cursor-pointer
                        ${campaignData.selectedAccounts.includes(influencer.channel_id)
                      ...
                      <div className="flex">
                        ...
                        <span className="text-white font-medium">
                          {campaignData.selectedAccounts.length > 0 
                            ? youtubeInfluencers
                                .filter(influencer => campaignData.selectedAccounts.includes(influencer.channel_id))
                                .map(influencer => influencer.channel_title)
                                .join(', ')
                            : 'None selected'}
                        </span>
                      </div>
